...
complete -c tunet-bash -s q -l quiet -d 'Suppress all log output'
complete -c tunet-bash -l pass -x -d 'When used with config, set passname instead of password'
complete -c tunet-bash -l output -d 'Output format' -x -a 'text json'
complete -c tunet-bash -s a -l auth -d 'Auth method' -x -a '4 6 auto generic'
