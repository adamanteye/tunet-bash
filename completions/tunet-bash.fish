...
complete -c tunet-bash -l pass -x -d 'When used with config, set pass-name instead of password'
complete -c tunet-bash -s a -l auth -d 'Auth method' -x -a '4 6 auto either'
complete -c tunet-bash -l version -d 'Print version'
complete -c tunet-bash -l curl-extra-args -d 'Pass extra arguments to curl' -x
